...
**Abgrenzung zur opt-in Automatik-Schicht (Execution-Layer).** "Keine aktive Routine" heisst: der **Governance-Core** (`.ai-workspace/`) feuert nichts von selbst. Davon zu unterscheiden ist die optionale, core-mitgelieferte Automatik-Schicht unter `.claude/` (Session-Hooks `boot_reload`, `recitation_nudge`, `daily_maintenance` -- alle default AUS -- plus das default-AN Erst-Start-Onboarding `first_run_onboarding`/`/start`; reversibel ueber `.claude/automation.flags.json`, gefuehrt durch `/uaw-automation`). Der `daily_maintenance`-Hook ist eine **aktive, aber opt-in Pflege-Routine in der Execution-Schicht**; er widerspricht "keine aktive Routine" nicht, weil der Governance-Core selbst weiterhin nichts mutiert und der Hook nur vorschlaegt (D-2026-06-06-02). Diese Schicht ist **kein Adapter** und **keine registrierte Maintenance-Routine** -- sie ist domain-neutrale Core-Execution (vgl. §11) und gehorcht denselben Sicherheits-Defaults: Aktivierung erfordert explizite User-Zustimmung, Deaktivierung ist jederzeit moeglich. Ein Adapter darf diese Schicht **nicht** stillschweigend umverdrahten, als Aktivierungs-Bypass nutzen oder als Abhaengigkeit voraussetzen; bringt ein Adapter eigene Routinen ein, laufen sie ueber die Registrierung in diesem §8, nicht ueber die Core-Automatik.
